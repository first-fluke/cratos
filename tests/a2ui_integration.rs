...
use cratos_tools::{
    register_builtins_with_config, BuiltinsConfig, ToolRegistry, ToolRunner, ExecutionOptions
};
...
async fn test_a2ui_tools_integration() {
    ...
    let canvas_state = Arc::new(
        CanvasState::new(session_manager.clone())
    ...
    assert!(registry.has("a2ui_render"), "a2ui_render tool should be registered");
    assert!(registry.has("a2ui_wait_event"), "a2ui_wait_event tool should be registered");
    ...
    let result = runner.execute_with_options(
        "a2ui_render", 
        args, 
    ...
    assert!(exec_result.result.success, "Tool returned failure: {:?}", exec_result.result.error);
    
    let output = exec_result.result.output;
    assert!(output.get("component_id").is_some(), "Output should contain component_id");
