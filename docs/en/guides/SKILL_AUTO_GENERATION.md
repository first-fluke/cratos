...
Cratos's **Skill Auto-generation System** learns from user tool usage patterns and automatically converts repetitive tasks into reusable skills. This is Cratos's key differentiator.

Generated skills feed into the Autonomous Agent's ReAct loop, where the LLM autonomously selects and executes skills following Plan-Act-Reflect principles. Cratos currently provides 23 built-in tools, and the skill system learns combinations of these tools to achieve higher-level automation.
...
|  +----------------------------------------------------------------+ |
|  |  - LLM autonomously selects tools via Plan-Act-Reflect          | |
|  |  - Leverages 23 built-in tools (cratos-tools)                   | |
|  |  - Soft failure: _diagnosis hint -> LLM finds alternatives      | |
|  |  - Consecutive failures: auto-inject [reflection] prompt        | |
|  +----------------------------------------------------------------+ |
+---------------------------------------------------------------------+
```
