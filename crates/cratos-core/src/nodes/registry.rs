...
impl NodeRegistry {
    ...
    pub async fn register(
        ...
        let capabilities_json = serde_json::to_string(&params.capabilities)
            .map_err(|e| NodeError::DatabaseError(format!("Failed to serialize capabilities: {}", e)))?;
        let commands_json = serde_json::to_string(&params.declared_commands)
            .map_err(|e| NodeError::DatabaseError(format!("Failed to serialize commands: {}", e)))?;
    ...
    pub async fn list_nodes(&self, auth: &AuthContext) -> Result<Vec<NodeSummary>, NodeError> {
        ...
        for row in rows {
            ...
            let capabilities = serde_json::from_str(&cap_str)
                .unwrap_or_default();
    ...
    fn map_row_to_node(&self, row: &sqlx::sqlite::SqliteRow) -> Result<Node, NodeError> {
        ...
        Ok(Node {
            ...
            name: row.get("name"),
            platform: serde_json::from_str(&format!("\"{}\"", platform_str)).unwrap_or(Platform::Other),
            ...
            last_seen: DateTime::parse_from_rfc3339(&last_seen_str)
                .map(|d| d.with_timezone(&Utc))
                .unwrap_or_else(|_| Utc::now()),
            connection_id: self.sessions.get(&device_id).and_then(|s| s.connection_id.clone()),
        })
    }
    ...
    pub async fn check_command(
        ...
        self.policy.is_allowed(command, &node.declared_commands)
    ...
    pub async fn check_liveness(&self) -> Result<(), NodeError> {
        ...
        sqlx::query("UPDATE nodes SET status = 'offline' WHERE status = 'online' AND last_seen < $1")
