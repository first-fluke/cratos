use std::sync::Arc;
use uuid::Uuid;
use crate::websocket::{CanvasState, BroadcastMessage};
...
// ...
    ...
    pub async fn get_or_create(&self, session_id: Uuid) -> Result<A2uiSession, String> {
        if self.state.session_manager.get_session(session_id).await.is_none() {
...
impl A2uiSession {
    ...
    pub async fn wait_event(&self, component_id: Uuid) -> Result<crate::a2ui::protocol::A2uiClientMessage, String> {
        ...
        loop {
            match rx.recv().await {
                Ok((session_id, msg)) => {
                    ...
                    if let crate::a2ui::protocol::A2uiClientMessage::Event { component_id: event_comp_id, .. } = &msg {
