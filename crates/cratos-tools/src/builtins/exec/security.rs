use super::config::{ExecConfig, ExecMode};
...
pub fn is_command_blocked(config: &ExecConfig, command: &str) -> bool {
    ...
    match config.mode {
        ExecMode::Strict => !config.allowed_commands.iter().any(|a| a == base_command),
        ExecMode::Permissive => {
            let is_builtin_blocked = DEFAULT_BLOCKED_COMMANDS.contains(&base_command);
            let is_network_blocked =
                !config.allow_network_commands && NETWORK_EXFIL_COMMANDS.contains(&base_command);
            let is_extra_blocked = config
