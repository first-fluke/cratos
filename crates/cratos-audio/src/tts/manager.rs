...
impl TtsManager {
    ...
    pub async fn synthesize(&self, text: &str, voice: &str, options: Option<TtsOptions>) -> Result<Bytes> {
            ...
            if backend.is_available() {
                match backend.synthesize(text, voice, &options).await {
                    ...
                    Err(e) => {
                        ...
                        warn!("Default backend {} failed: {}, trying fallback", self.default_backend, e);
                    }
                }
            }
        }
